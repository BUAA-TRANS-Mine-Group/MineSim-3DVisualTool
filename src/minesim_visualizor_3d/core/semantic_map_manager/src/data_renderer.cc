...
DataRenderer::DataRenderer(SemanticMapManager *smm_ptr) : p_semantic_map_manager_(smm_ptr) {
    ...
    std::array<int, 2> map_size = {{obstacle_map_info_.height, obstacle_map_info_.width}};
    std::array<decimal_t, 2> map_resl = {{obstacle_map_info_.resolution, obstacle_map_info_.resolution}};
    ...
    printf("#log# [DataRenderer] Initialization finished\n");
}

ErrorType DataRenderer::Render(const double &time_stamp, const common::LaneNet &lane_net, const common::VehicleSet &vehicle_set,
    ...
    TicToc timer;
    FakeMapper();
    // printf("#log# [RayCasting]Time cost: %lf ms\n", timer.toc());

    p_semantic_map_manager_->UpdateSemanticMap(time_stamp_, ego_vehicle_, whole_lane_net_, surrounding_lane_net_, *p_obstacle_grid_, obs_grids_,
                                               surrounding_vehicles_);
...
ErrorType DataRenderer::InjectObservationNoise() {
    ...
    if (cnt_random_ == 10) {
        ...
        std::shuffle(surrounding_ids.begin(), surrounding_ids.end(), random_engine_);
        ...
        for (auto &v : surrounding_vehicles_.vehicles) {
            ...
            Vec2f augmented_position = Vec2f(original_position.x() + lateral_position_noise * sin(angle) + long_position_noise * cos(angle),
                                             original_position.y() - lateral_position_noise * cos(angle) + long_position_noise * sin(angle));
...
ErrorType DataRenderer::GetObstacleMap(const common::ObstacleSet &obstacle_set) {
    ...
    cv::Mat grid_mat = cv::Mat(obstacle_map_info_.height, obstacle_map_info_.width, CV_MAKETYPE(cv::DataType<ObstacleMapType>::type, 1),
...
ErrorType DataRenderer::RayCastingOnObstacleMap() {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        roguelike_ray_casting::RasterizeFOVOctant(coord[0], coord[1], r_idx_max, p_obstacle_grid_->dims_size(0), p_obstacle_grid_->dims_size(1), i,
                                                  p_obstacle_grid_->data_ptr(), render_mat.data(), &obs_coord_set);
...
ErrorType DataRenderer::GetSurroundingLaneNet(const common::LaneNet &lane_net) {
    ...
    for (auto iter = lane_net.lane_set.begin(); iter != lane_net.lane_set.end(); ++iter) {
        for (int i = 0; i < static_cast<int>(iter->second.lane_points.size()); ++i) {
    ...
    kdtree_lane_net_ = std::make_shared<KdTreeFor2dPointVec>(2, lane_net_pts_, nanoflann::KDTreeSingleIndexAdaptorParams(10));
    ...
    const decimal_t search_radius = surrounding_search_radius_ * surrounding_search_radius_ * 4;
    ...
    for (const auto id : matched_lane_id_set) {
        surrounding_lane_net_.lane_set.insert(std::pair<int, common::LaneRaw>(id, lane_net.lane_set.at(id)));
    }
    return kSuccess;
}
