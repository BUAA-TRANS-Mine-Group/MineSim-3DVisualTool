...
// 语义管理器，设置agent相关属性
SemanticMapManager::SemanticMapManager(const int &id, const std::string &agent_config_path)
    : ego_id_(id), agent_config_path_(agent_config_path) { // 构造函数后加冒号是初始化表达式
...
SemanticMapManager::SemanticMapManager(const int &id, const decimal_t surrounding_search_radius, bool enable_openloop_prediction,
...
ErrorType SemanticMapManager::UpdateSemanticMap(const double &time_stamp, const common::Vehicle &ego_vehicle, const common::LaneNet &whole_lane_net,
...
ErrorType SemanticMapManager::SaveMapToLog() {
    ...
    record_file_stream.open(agent_config_info_.log_file, std::ofstream::out | std::ofstream::app);
    ...
    for (auto &v : vehicle_set.vehicles) {
        record_file_stream << v.second.state().time_stamp << "," << v.first << "," << v.second.state().vec_position[0] << ","
                           << v.second.state().vec_position[1] << "," << v.second.state().velocity << "," << v.second.state().acceleration << ","
                           << v.second.state().angle << "," << v.second.state().curvature << "," << v.second.state().steer << std::endl;
    }
    return kSuccess;
}

ErrorType SemanticMapManager::NaiveRuleBasedLateralBehaviorPrediction(const common::Vehicle &vehicle, const int nearest_lane_id,
    ...
    SemanticLane nearest_lane = semantic_lane_set_.semantic_lanes.at(nearest_lane_id);
    ...
    if (use_right_hand_axis_) {
        if (fs.vec_dt[0] > lat_distance_threshold && fs.vec_dt[1] > lat_vel_threshold && nearest_lane.l_lane_id != kInvalidLaneId &&
            nearest_lane.l_change_avbl) {
            prob_lcl = 1.0;
            printf("[NaivePrediction]vehicle %d lane id %d, lat d %lf lat dd %lf, "
                   "behavior "
                   "lcl.\n",
                   vehicle.id(), nearest_lane_id, fs.vec_dt[0], fs.vec_dt[1]);
        } else if (fs.vec_dt[0] < -lat_distance_threshold && fs.vec_dt[1] < -lat_vel_threshold && nearest_lane.r_lane_id != kInvalidLaneId &&
        ...
        if (fs.vec_dt[0] > lat_distance_threshold && fs.vec_dt[1] > lat_vel_threshold && nearest_lane.r_lane_id != kInvalidLaneId &&
            nearest_lane.r_change_avbl) {
            prob_lcr = 1.0;
            printf("[NaivePrediction]vehicle %d lane id %d, lat d %lf lat dd %lf, "
                   "behavior "
                   "lcr.\n",
                   vehicle.id(), nearest_lane_id, fs.vec_dt[0], fs.vec_dt[1]);
        } else if (fs.vec_dt[0] < -lat_distance_threshold && fs.vec_dt[1] < -lat_vel_threshold && nearest_lane.l_lane_id != kInvalidLaneId &&
                   nearest_lane.l_change_avbl) {
            prob_lcl = 1.0;
            printf("[NaivePrediction]vehicle %d lane id %d, lat d %lf lat dd %lf, "
                   "behavior "
...
ErrorType SemanticMapManager::MobilRuleBasedBehaviorPrediction(const common::Vehicle &vehicle, const common::VehicleSet &nearby_vehicles,
    ...
    std::vector<LateralBehavior> behaviors{LateralBehavior::kLaneKeeping, LateralBehavior::kLaneChangeLeft, LateralBehavior::kLaneChangeRight};

    for (const auto &behavior : behaviors) {
        // ~ Prepare lane
        common::Lane ref_lane;
        GetRefLaneForStateByBehavior(vehicle.state(), std::vector<int>(), behavior, lane_radius, lane_radius, false, &ref_lane);
        ...
        GetLeadingAndFollowingVehiclesFrenetStateOnLane(ref_lane, vehicle.state(), nearby_vehicles, &has_leading_vehicle, &leading_vehicle,
                                                        &leading_frenet_state, &has_following_vehicle, &following_vehicle, &following_frenet_state);
    ...
    if (common::MobilBehaviorPrediction::LateralBehaviorPrediction(vehicle, lanes, leading_vehicles, leading_frenet_states, following_vehicles,
                                                                   follow_frenet_states, nearby_vehicles, res) != kSuccess) {
        return kWrongStatus;
    }
    return kSuccess;
}

ErrorType SemanticMapManager::GetLeadingAndFollowingVehiclesFrenetStateOnLane(const common::Lane &ref_lane, const common::State &ref_state,
    ...
    GetLeadingVehicleOnLane(ref_lane, ref_state, vehicle_set, lat_range, leading_vehicle, &distance_residual_ratio);
    GetFollowingVehicleOnLane(ref_lane, ref_state, vehicle_set, lat_range, following_vehicle);
    ...
    *has_following_vehicle = false;
    ...
    if (leading_vehicle->id() != kInvalidAgentId) {
        if (stf.GetFrenetStateFromState(leading_vehicle->state(), leading_fs) == kSuccess) {
    ...
    if (following_vehicle->id() != kInvalidAgentId) {
        if (stf.GetFrenetStateFromState(following_vehicle->state(), following_fs) == kSuccess) {
...
ErrorType SemanticMapManager::GetEgoNearestLaneId(int *ego_lane_id) const {
    ...
    if (GetNearestLaneIdUsingState(ego_vehicle_.state().ToXYTheta(), std::vector<int>(), &nearest_lane_id, &distance, &arclen) != kSuccess) {
        return kWrongStatus;
    }
...
ErrorType SemanticMapManager::UpdateSemanticVehicles() {
    ...
    for (const auto &v : surrounding_vehicles_.vehicles) {
        ...
        GetNearestLaneIdUsingState(semantic_vehicle.vehicle.state().ToXYTheta(), std::vector<int>(), &semantic_vehicle.nearest_lane_id,
                                   &semantic_vehicle.dist_to_lane, &semantic_vehicle.arc_len_onlane);

        NaiveRuleBasedLateralBehaviorPrediction(semantic_vehicle.vehicle, semantic_vehicle.nearest_lane_id, &semantic_vehicle.probs_lat_behaviors);
        semantic_vehicle.probs_lat_behaviors.GetMaxProbBehavior(&semantic_vehicle.lat_behavior);

        decimal_t max_backward_len = 10.0;
        decimal_t forward_lane_len = std::max(semantic_vehicle.vehicle.state().velocity * 10.0, 50.0);
        GetRefLaneForStateByBehavior(semantic_vehicle.vehicle.state(), std::vector<int>(), semantic_vehicle.lat_behavior, forward_lane_len,
                                     max_backward_len, false, &semantic_vehicle.lane);

        semantic_vehicles_tmp.semantic_vehicles.insert(std::pair<int, common::SemanticVehicle>(semantic_vehicle.vehicle.id(), semantic_vehicle));
    }
    { semantic_surrounding_vehicles_.semantic_vehicles.swap(semantic_vehicles_tmp.semantic_vehicles); }
    return kSuccess;
}
...
ErrorType SemanticMapManager::OpenloopTrajectoryPrediction() {
    ...
    for (const auto &p_sv : semantic_surrounding_vehicles_.semantic_vehicles) {
        ...
        TrajectoryPredictionForVehicle(semantic_vehicle.vehicle, semantic_vehicle.lane, pred_time_, pred_step_, &traj);
...
ErrorType SemanticMapManager::UpdateSemanticLaneSet() {
    ...
    // Update semantic lane set
    {
        for (const auto &pe : surrounding_lane_net_.lane_set) {
            ...
            if (common::LaneGenerator::GetLaneBySamplePoints(samples, &semantic_lane.lane) != kSuccess) {
                continue;
            }

            semantic_lane_set_.semantic_lanes.insert(std::pair<int, common::SemanticLane>(semantic_lane.id, semantic_lane));
        }
    }
    // Check the consistency of semantic map
    {
        for (auto &semantic_lane : semantic_lane_set_.semantic_lanes) {
            if (semantic_lane.second.l_change_avbl) {
                auto l_it = semantic_lane_set_.semantic_lanes.find(semantic_lane.second.l_lane_id);
            ...
            if (semantic_lane.second.r_change_avbl) {
                auto r_it = semantic_lane_set_.semantic_lanes.find(semantic_lane.second.r_lane_id);
            ...
            for (auto it = semantic_lane.second.father_id.begin(); it < semantic_lane.second.father_id.end();) {
            ...
            for (auto it = semantic_lane.second.child_id.begin(); it < semantic_lane.second.child_id.end();) {
...
ErrorType SemanticMapManager::UpdateLocalLanesAndFastLut() {
    ...
    if (kSuccess != GetNearestLaneIdUsingState(ego_state.ToXYTheta(), std::vector<int>(), &cur_lane_id, &dist_tmp, &arc_len_tmp)) {
        return kWrongStatus;
    }
    ...
    for (const auto root_id : root_lane_ids) {
        decimal_t arc_len;
        semantic_lane_set_.semantic_lanes.at(root_id).lane.GetArcLengthByVecPosition(ego_state.vec_position, &arc_len);
        decimal_t length_remain = semantic_lane_set_.semantic_lanes.at(root_id).length - arc_len;
        // printf("#log# [XXX]root: %d, arc_len: %lf, remain: %lf\n", root_id, arc_len,
        //        length_remain);

        // ~ Get forward lane paths
        std::vector<std::vector<int>> all_paths_forward;
        GetAllForwardLaneIdPathsWithMinimumLengthByRecursion(root_id, length_remain, 0.0, std::vector<int>(), &all_paths_forward);

        // for (const auto path : all_paths_forward) {
        //   printf("#log# [XXX]Forward:  ");
        ...
        GetAllBackwardLaneIdPathsWithMinimumLengthByRecursion(root_id, arc_len, 0.0, std::vector<int>(), &all_paths_backward);
        // for (const auto path : all_paths_backward) {
        //   printf("#log# [XXX]Backward:  ");
        //   for (const auto id : path) {
        //     printf("#log# %d ", id);
        //   }
        //   printf("#log# \n");
        ...
        for (const auto &path : assembled_paths) {
            common::Lane lane;
            if (kSuccess != GetLocalLaneUsingLaneIds(ego_state, path, local_lane_length_forward_, local_lane_length_backward_, true, &lane)) {
                continue;
            }
            int local_id = local_lane_cnt++;
            local_lanes_.insert(std::pair<int, common::Lane>(local_id, lane));

            local_to_segment_lut_.insert(std::pair<int, std::vector<int>>(local_id, path));
    ...
    //   printf("#log# [XXX]segment: %d - local: ", entry.first);
    //   for (const auto& id: entry.second) {
    //     printf("#log# %d ", id);
    //   }
    //   printf("#log# \n");
...
void SemanticMapManager::GetAllForwardLaneIdPathsWithMinimumLengthByRecursion(const decimal_t &node_id, const decimal_t &node_length,
    ...
    if (node_aggre_length >= local_lane_length_forward_ || whole_lane_net_.lane_set.at(node_id).child_id.empty()) {
        // stop recursion
        ...
        for (const auto child_id : child_ids) {
            ...
            GetAllForwardLaneIdPathsWithMinimumLengthByRecursion(child_id, child_length, node_aggre_length, path, all_paths);
        }
    }
}

void SemanticMapManager::GetAllBackwardLaneIdPathsWithMinimumLengthByRecursion(const decimal_t &node_id, const decimal_t &node_length,
                                                                               const decimal_t &aggre_length, const std::vector<int> &path_to_node,
                                                                               std::vector<std::vector<int>> *all_paths) {
    // * Traverse BACKWARD
    // check cycle here?
    ...
    if (node_aggre_length >= local_lane_length_backward_ || whole_lane_net_.lane_set.at(node_id).father_id.empty()) {
        // stop recursion
        ...
        for (const auto father_id : father_ids) {
            ...
            GetAllBackwardLaneIdPathsWithMinimumLengthByRecursion(father_id, father_length, node_aggre_length, path, all_paths);
        }
    }
}

ErrorType SemanticMapManager::GetDistanceToLanesUsing3DofState(const Vec3f &state,
                                                               std::set<std::tuple<decimal_t, decimal_t, decimal_t, int>> *res) const {
    for (const auto &p : semantic_lane_set_.semantic_lanes) {
        decimal_t arc_len;
        p.second.lane.GetArcLengthByVecPosition(Vec2f(state(0), state(1)), &arc_len);
        ...
        //   printf("#log# [XXX]lane_id: %d, arc_len1: %lf, arc_len2: %lf\n", p.second.id,
        //          arc_len, arc_len2);
        // }
        ...
        res->insert(std::tuple<decimal_t, decimal_t, decimal_t, int>(dist, arc_len, angle_diff, p.second.id));
    }
#if 0
...
ErrorType SemanticMapManager::CheckCollisionUsingState(const common::VehicleParam &param_a, const common::State &state_a,
                                                       const common::VehicleParam &param_b, const common::State &state_b, bool *res) {
    common::OrientedBoundingBox2D obb_a, obb_b;
    common::SemanticsUtils::GetOrientedBoundingBoxForVehicleUsingState(param_a, state_a, &obb_a);
    common::SemanticsUtils::GetOrientedBoundingBoxForVehicleUsingState(param_b, state_b, &obb_b);
    *res = common::ShapeUtils::CheckIfOrientedBoundingBoxIntersect(obb_a, obb_b);
    return kSuccess;
}

ErrorType SemanticMapManager::CheckCollisionUsingStateAndVehicleParam(const common::VehicleParam &vehicle_param, const common::State &state,
                                                                      bool *res) {
    // check static collision
    {
        ...
        common::ShapeUtils::GetVerticesOfOrientedBoundingBox(vehicle.RetOrientedBoundingBox(), &vertices);
    ...
    if (agent_config_info_.enable_openloop_prediction) {
        for (const auto &v : semantic_surrounding_vehicles_.semantic_vehicles) {
            ...
            int access_index = std::round((state_stamp - obstacle_init_stamp) / pred_step_);
            int num_pred_states = static_cast<int>(openloop_pred_traj.size());
            if (access_index < 0 || access_index >= num_pred_states)
                continue;
            bool is_collision = false;
            CheckCollisionUsingState(vehicle_param, state, v.second.vehicle.param(), openloop_pred_traj[access_index], &is_collision);
            if (is_collision) {
                *res = is_collision;
                return kSuccess;
            }
...
ErrorType SemanticMapManager::CheckCollisionUsingGlobalPosition(const Vec2f &p_w, bool *res) const {
    std::array<decimal_t, 2> p = {{p_w(0), p_w(1)}};
    return obstacle_map_.CheckIfEqualUsingGlobalPosition(p, GridMap2D::OCCUPIED, res);
}

ErrorType SemanticMapManager::GetObstacleMapValueUsingGlobalPosition(const Vec2f &p_w, ObstacleMapType *res) {
    std::array<decimal_t, 2> p = {{p_w(0), p_w(1)}};
    return obstacle_map_.GetValueUsingGlobalPosition(p, res);
}

ErrorType SemanticMapManager::IsTopologicallyReachable(const int lane_id, const std::vector<int> &path, int *num_lane_changes, bool *res) const {
    if (semantic_lane_set_.semantic_lanes.count(lane_id) == 0) {
        printf("#log# [IsTopologicallyReachable]fail to get lane id %d.\n", lane_id);
        return kWrongStatus;
    }
...
ErrorType SemanticMapManager::GetNearestLaneIdUsingState(const Vec3f &state, const std::vector<int> &navi_path, int *id, decimal_t *distance,
    ...
    if (lanes_in_dist.empty()) {
        printf("#log# [GetNearestLaneIdUsingState]No nearest lane found.\n");
        return kWrongStatus;
    }
    ...
    std::set<std::tuple<decimal_t, decimal_t, decimal_t, int>> lanes_in_angle_diff;
    for (const auto &ele : lanes_in_dist) {
        if (std::get<0>(ele) > nearest_lane_range_) {
            break;
        }
        lanes_in_angle_diff.insert(
            std::tuple<decimal_t, decimal_t, decimal_t, int>(fabs(std::get<2>(ele)), std::get<0>(ele), std::get<1>(ele), std::get<3>(ele)));
    }
    if (lanes_in_angle_diff.empty() || std::get<0>(*lanes_in_angle_diff.begin()) > kPi / 2) {
    ...
    *arc_len = std::get<2>(*lanes_in_angle_diff.begin());
    ...
    if (std::get<3>(*lanes_in_angle_diff.begin()) != std::get<3>(*lanes_in_dist.begin())) {
        // printf(
    ...
    // *distance = std::get<0>(*lanes_in_dist.begin());
    // *arc_len = std::get<1>(*lanes_in_dist.begin());
    // printf("#log# [GetNearestLaneIdUsingState]angle_diff: %lf, dist: %lf, id: %d\n",
...
ErrorType SemanticMapManager::UpdateKeyVehicles() {
    ...
    if (GetNearestLaneIdUsingState(ego_vehicle_.state().ToXYTheta(), std::vector<int>(), &cur_lane_id, &cur_lane_dist, &cur_arc_len) != kSuccess) {
        return kWrongStatus;
    }
    ...
    // Find key lane ids
    {
        ...
        if (whole_lane_net_.lane_set.at(cur_lane_id).l_change_avbl) {
            key_lane_ids.insert(std::pair<int, decimal_t>(whole_lane_net_.lane_set.at(cur_lane_id).l_lane_id, -cur_arc_len));
            mid_lane_ids.push_back(whole_lane_net_.lane_set.at(cur_lane_id).l_lane_id);
        }
        if (whole_lane_net_.lane_set.at(cur_lane_id).r_change_avbl) {
            key_lane_ids.insert(std::pair<int, decimal_t>(whole_lane_net_.lane_set.at(cur_lane_id).r_lane_id, -cur_arc_len));
            mid_lane_ids.push_back(whole_lane_net_.lane_set.at(cur_lane_id).r_lane_id);
        }

        // successors
        decimal_t len_front = whole_lane_net_.lane_set.at(cur_lane_id).length - cur_arc_len;
        for (const int &id : mid_lane_ids) {
            ...
            while (!id_to_expand.empty()) {
                ...
                std::vector<int> succ_ids = whole_lane_net_.lane_set.at(id_expand).child_id;
                for (const auto &succ_id : succ_ids) {
                    key_lane_ids.insert(std::pair<int, decimal_t>(succ_id, len_sum));
                    decimal_t len_tmp = whole_lane_net_.lane_set.at(succ_id).length + len_expand;
        ...
        for (const auto &id : mid_lane_ids) {
            ...
            while (!id_to_expand.empty()) {
                ...
                int id_expand = it->second;
                id_to_expand.erase(it);
                std::vector<int> pred_ids = whole_lane_net_.lane_set.at(id_expand).father_id;
                for (const auto &pred_id : pred_ids) {
                    decimal_t len_tmp = -whole_lane_net_.lane_set.at(pred_id).length + len_expand;
    ...
    decimal_t t_pl = 5.0;
    decimal_t t_comfort = std::max(t_pl, ego_vehicle_.state().velocity / dec_comfort);
        ...
        for (const auto v : semantic_surrounding_vehicles_.semantic_vehicles) {
            ...
            if (it != key_lane_ids.end()) {
                ...
                if (dist >= 0 && fabs(dist) < front_range) {
                    int v_id = v.first;
                    if (v.second.nearest_lane_id == cur_lane_id && v.second.arc_len_onlane < cur_arc_len)
                        continue;
                    key_vehicle_ids_.push_back(v_id);
                    semantic_key_vehicles_.semantic_vehicles.insert(
                        std::pair<int, common::SemanticVehicle>(v_id, semantic_surrounding_vehicles_.semantic_vehicles.at(v_id)));
                    key_vehicles_.vehicles.insert(
                        std::pair<int, common::Vehicle>(v_id, semantic_surrounding_vehicles_.semantic_vehicles.at(v_id).vehicle));
                }
                // * Rear
                decimal_t s_margin = std::max(20.0, fabs(v.second.vehicle.state().velocity) * t_pl);
...
ErrorType SemanticMapManager::GetLocalLaneSamplesByState(const common::State &state, const int lane_id, const std::vector<int> &navi_path,
                                                         const decimal_t max_reflane_dist, const decimal_t max_backward_dist,
                                                         vec_Vecf<2> *samples) const {
    if (semantic_lane_set_.semantic_lanes.count(lane_id) == 0) {
        printf("#log# [GetLocalLaneSamplesByState]fail to get lane id %d.\n", lane_id);
        return kWrongStatus;
    }
    ...
    std::vector<int> ids_back;
    {
        if (arclen < max_backward_dist) {
            ...
            while (accum_dist_backward < max_backward_dist) {
                std::vector<int> father_ids = semantic_lane_set_.semantic_lanes.at(id_tmp).father_id;
                if (!father_ids.empty()) {
                    // TODO: double check the logic for front
                    int father_id = father_ids.front();
                    for (auto &id : father_ids) {
                        if (std::find(navi_path.begin(), navi_path.end(), id) != navi_path.end()) {
                            father_id = id;
                            break;
                        }
                    }
    ...
    {
        decimal_t dist_remain_target_lane = semantic_lane_set_.semantic_lanes.at(lane_id).length - arclen;
        if (dist_remain_target_lane < max_reflane_dist) {
            ...
            while (accum_dist_forward < max_reflane_dist) {
                std::vector<int> child_ids = semantic_lane_set_.semantic_lanes.at(id_tmp).child_id;
                // TODO: double check the logic for front
                if (!child_ids.empty()) {
                    ...
                    accum_dist_forward += semantic_lane_set_.semantic_lanes.at(child_id).length;
    ...
    for (const auto &id : lane_id_all) {
        if (raw_samples.empty() && (int)surrounding_lane_net_.lane_set.at(id).lane_points.size() > 0) {
    ...
    SampleLane(long_lane, sample_start, sample_start + forward_sample_len + std::min(accum_dist_backward, max_backward_dist), 1.0, samples,
               &acc_dist_tmp);

    return kSuccess;
}

ErrorType SemanticMapManager::GetLocalLaneUsingLaneIds(const common::State &state, const std::vector<int> &lane_ids, const decimal_t forward_length,
                                                       const decimal_t backward_length, const bool &is_high_quality, common::Lane *lane) {
    vec_Vecf<2> raw_samples;
    for (const auto &id : lane_ids) {
        if (raw_samples.empty() && whole_lane_net_.lane_set.at(id).lane_points.size() > 0) {
            raw_samples.push_back(whole_lane_net_.lane_set.at(id).lane_points[0]);
        }
        for (int i = 1; i < whole_lane_net_.lane_set.at(id).lane_points.size(); ++i) {
...
ErrorType SemanticMapManager::GetRefLaneForStateByBehavior(const common::State &state, const std::vector<int> &navi_path,
                                                           const LateralBehavior &behavior, const decimal_t &max_forward_len,
                                                           const decimal_t &max_back_len, const bool is_high_quality, common::Lane *lane) const {
    Vec3f state_3dof(state.vec_position(0), state.vec_position(1), state.angle);
    int current_lane_id;
    decimal_t distance_to_lane;
    decimal_t arc_len;
    if (GetNearestLaneIdUsingState(state_3dof, navi_path, &current_lane_id, &distance_to_lane, &arc_len) != kSuccess) {
        printf("#log# [GetRefLaneForStateByBehavior]Cannot get nearest lane.\n");
        return kWrongStatus;
    }
    ...
    if (GetLocalLaneSamplesByState(state, target_lane_id, navi_path, max_forward_len, max_back_len, &samples) != kSuccess) {
        printf("#log# [GetRefLaneForStateByBehavior]Cannot get local lane samples.\n");
        return kWrongStatus;
    }
...
ErrorType SemanticMapManager::GetLaneBySampledPoints(const vec_Vecf<2> &samples, const bool &is_high_quality, common::Lane *lane) const {
    if (is_high_quality) {
        double d = 0.0;
        ...
        Eigen::ArrayXf breaks = Eigen::ArrayXf::LinSpaced(num_segments, para.front(), para.back());

        const decimal_t regulator = (double)1e6;
        if (common::LaneGenerator::GetLaneBySampleFitting(samples, para, breaks, regulator, lane) != kSuccess) {
            return kWrongStatus;
        }
    } else {
        if (common::LaneGenerator::GetLaneBySamplePoints(samples, lane) != kSuccess) {
            return kWrongStatus;
        }
    }
    return kSuccess;
...
ErrorType SemanticMapManager::SampleLane(const common::Lane &lane, const decimal_t &s0, const decimal_t &s1, const decimal_t &step,
...
ErrorType SemanticMapManager::GetTargetLaneId(const int lane_id, const LateralBehavior &behavior, int *target_lane_id) const {
    auto it = semantic_lane_set_.semantic_lanes.find(lane_id);
    if (it == semantic_lane_set_.semantic_lanes.end()) {
        return kWrongStatus;
    } else {
        if (behavior == common::LateralBehavior::kLaneKeeping || behavior == common::LateralBehavior::kUndefined) {
...
ErrorType SemanticMapManager::GetLeadingVehicleOnLane(const common::Lane &ref_lane, const common::State &ref_state,
                                                      const common::VehicleSet &vehicle_set, const decimal_t &lat_range,
                                                      common::Vehicle *leading_vehicle, decimal_t *distance_residual_ratio) const {
    /**
     *    [>>>>]    ------->    [>>>>]
     *      | offset               |
     *      |                      |
    ...
    for (decimal_t s = ref_fs.vec_s[0] + resolution + search_lon_offset; s < ref_fs.vec_s[0] + max_forward_search_dist + search_lon_offset;
         s += resolution) {
...
ErrorType SemanticMapManager::GetFollowingVehicleOnLane(const common::Lane &ref_lane, const common::State &ref_state,
                                                        const common::VehicleSet &vehicle_set, const decimal_t &lat_range,
                                                        common::Vehicle *following_vehicle) const {
    common::StateTransformer stf(ref_lane);
    common::FrenetState ref_fs;
    if (stf.GetFrenetStateFromState(ref_state, &ref_fs) != kSuccess) {
        // printf("#log# [FollowingVehicleOnLane]Cannot get ref state frenet state.\n");
        return kWrongStatus;
    }

    const decimal_t lane_width_tol = lat_range;
    decimal_t max_backward_search_dist = std::min(ref_fs.vec_s[0] - ref_lane.begin(), 100.0);
    ...
    for (decimal_t delta_s = lane_width_tol / 1.4; delta_s < max_backward_search_dist - 2.0 * lane_width_tol; delta_s += lane_width_tol / 1.4) {
        ref_lane.GetPositionByArcLength(ref_fs.vec_s[0] - delta_s, &lane_pt);
        for (auto &entry : vehicle_set.vehicles) {
            if (entry.second.id() == kInvalidAgentId)
                continue;
            if ((lane_pt - entry.second.state().vec_position).norm() < lane_width_tol) {
...
ErrorType SemanticMapManager::GetSpeedLimit(const State &state, const Lane &lane, decimal_t *speed_limit) const {
    return traffic_singal_manager_.GetSpeedLimit(state, lane, speed_limit);
}

ErrorType SemanticMapManager::GetTrafficStoppingState(const State &state, const Lane &lane, State *stopping_state) const {
    return traffic_singal_manager_.GetTrafficStoppingState(state, lane, stopping_state);
}

bool SemanticMapManager::IsLocalLaneContainsLane(const int &local_lane_id, const int &seg_lane_id) const {
    if (!has_fast_lut_)
        return false;
...
ErrorType SemanticMapManager::GetDistanceOnLaneNet(const int &lane_id_0, const decimal_t &arc_len_0, const int &lane_id_1, const decimal_t &arc_len_1,
