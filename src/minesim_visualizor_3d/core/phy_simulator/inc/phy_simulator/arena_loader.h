...
class ArenaLoader {
  public:
     ...
     * @param lane_net_path
     */
    ArenaLoader(const std::string &vehicle_set_path, const std::string &map_path, const std::string &lane_net_path);
    ...
    /**
     * @brief Parse map borderline info from json
     *
     * 此处， Obstacle 指的是包含地图边界 障碍物 和其它静止障碍物。
     * @param p_obstacle_set
     * @return true Parsing success
     * @return false Parsing failed
     */
    ErrorType ParseMapInfo(common::ObstacleSet *p_border_set, bool use_actual_heights);
     ...
     * @return false Parsing failed
     */
    ErrorType ParseLaneNetInfo(common::LaneNet *p_lane_net, bool use_actual_heights);
