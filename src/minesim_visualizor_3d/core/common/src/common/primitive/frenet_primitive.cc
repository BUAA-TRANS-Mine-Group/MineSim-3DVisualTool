...
ErrorType FrenetPrimitive::Connect(const FrenetState &fs0, const FrenetState &fs1, const decimal_t stamp, const decimal_t T,
                                   bool is_lateral_independent) {
    is_lateral_independent_ = is_lateral_independent;

    poly_s_.GetJerkOptimalConnection(fs0.vec_s(0), fs0.vec_s(1), fs0.vec_s(2), fs1.vec_s(0), fs1.vec_s(1), fs1.vec_s(2), T);

    if (is_lateral_independent) {
        poly_d_.GetJerkOptimalConnection(fs0.vec_dt(0), fs0.vec_dt(1), fs0.vec_dt(2), fs1.vec_dt(0), fs1.vec_dt(1), fs1.vec_dt(2), T);
    } else {
        if (fs1.vec_s[0] - fs0.vec_s[0] < kSmallDistanceThreshold_) {
            const decimal_t virtual_large_distance = 100.0;
            poly_d_.GetJerkOptimalConnection(fs0.vec_ds(0), fs0.vec_ds(1), fs0.vec_ds(2), fs1.vec_ds(0), fs1.vec_ds(1), fs1.vec_ds(2),
                                             virtual_large_distance);
        } else {
            poly_d_.GetJerkOptimalConnection(fs0.vec_ds(0), fs0.vec_ds(1), fs0.vec_ds(2), fs1.vec_ds(0), fs1.vec_ds(1), fs1.vec_ds(2),
                                             fs1.vec_s[0] - fs0.vec_s[0]);
        }
    }
...
ErrorType FrenetPrimitive::Propagate(const FrenetState &fs0, const Vecf<2> &u, const decimal_t stamp, const decimal_t T) {
...
ErrorType FrenetPrimitive::GetFrenetState(const decimal_t t_global, FrenetState *fs) const {
    if (duration_ < kEPS)
        return kWrongStatus;
    auto t = t_global - stamp_;
    if (is_lateral_independent_) {
        fs->Load(Vecf<3>(poly_s_.evaluate(t, 0), poly_s_.evaluate(t, 1), poly_s_.evaluate(t, 2)),
                 Vecf<3>(poly_d_.evaluate(t, 0), poly_d_.evaluate(t, 1), poly_d_.evaluate(t, 2)), FrenetState::kInitWithDt);
        ...
        fs->Load(Vecf<3>(s, poly_s_.evaluate(t, 1), poly_s_.evaluate(t, 2)),
                 Vecf<3>(poly_d_.evaluate(st, 0), poly_d_.evaluate(st, 1), poly_d_.evaluate(st, 2)), FrenetState::kInitWithDs);
        fs->time_stamp = t_global;
    }
    return kSuccess;
}

ErrorType FrenetPrimitive::GetFrenetStateSamples(const decimal_t step, const decimal_t offset, vec_E<FrenetState> *fs_vec) const {
    FrenetState fs;
    fs_vec->clear();
