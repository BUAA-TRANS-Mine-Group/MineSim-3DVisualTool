...
class TrafficSignalManager {
  public:
    ...
    inline vec_E<SpeedLimit> speed_limit_list() const { return speed_limit_list_; }
    inline vec_E<TrafficLight> traffic_light_list() const { return traffic_light_list_; }

  private:
    ErrorType CheckIntersectionTypeWithSignal(const common::FrenetState &fs, const Lane &lane, const common::TrafficSignal &signal,
                                              IntersectionType *intersection_type, decimal_t *dist_to_startpt, decimal_t *dist_to_endpt) const;
