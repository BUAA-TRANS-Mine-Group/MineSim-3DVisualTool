...
ErrorType SplineGenerator<N_DEG, N_DIM>::GetCubicSplineBySampleInterpolation(const vec_Vecf<N_DIM> &samples, const std::vector<decimal_t> &para,
                                                                             SplineType *spline) {
    if (samples.size() < 3) {
...
template <int N_DEG, int N_DIM>
ErrorType SplineGenerator<N_DEG, N_DIM>::GetQuinticSplineBySampleFitting(const vec_Vecf<N_DIM> &samples, const std::vector<decimal_t> &para,
    ...
    // timer.tic();
    // prepare A
    Eigen::SparseMatrix<double, Eigen::RowMajor> A(N_DIM * num_samples, N_DIM * num_segments * num_order);
    ...
    // prepare W
    Eigen::DiagonalMatrix<double, Eigen::Dynamic> W(N_DIM * num_segments * num_order);
        ...
        for (int j = 0; j < num_order; j++) {
            for (int d = 0; d < N_DIM; d++) {
                if (j < regulate_top_degree)
                    weight(d * num_segments * num_order + n * num_order + j) = regulator / pow(10, j);
            }
        }
    }
    W.diagonal() = weight;
    ...
    Eigen::SparseMatrix<double, Eigen::RowMajor> C(N_DIM * num_connections * num_continuity, N_DIM * num_segments * num_order);
    C.reserve(Eigen::VectorXi::Constant(N_DIM * num_connections * num_continuity, 2 * num_order));
    {
        int idx, idy_l, idy_r;
        double val_l, val_r;
            ...
            for (int c = 0; c < num_continuity; c++) {
                for (int j = 0; j < num_order; j++) {
                    if (j <= num_order - 1 - c) {
                        ...
                        val_r = -pow(0.0, num_order - j - 1 - c) / fac(num_order - j - 1 - c);
                        for (int d = 0; d < N_DIM; d++) {
...
template <int N_DEG, int N_DIM>
ErrorType SplineGenerator<N_DEG, N_DIM>::GetWaypointsFromPositionSamples(const vec_Vecf<N_DIM> &samples, const std::vector<decimal_t> &para,
...
template <int N_DEG, int N_DIM>
ErrorType SplineGenerator<N_DEG, N_DIM>::GetSplineFromStateVec(const std::vector<decimal_t> &para, const vec_E<State> &state_vec,
                                                               SplineType *spline) {
    if (N_DEG < 5) {
    ...
    for (int i = 0; i < num_pts - 1; i++) {
        ...
        for (int d = 0; d < N_DIM; d++) {
            if (d <= 1) {
                (*spline)(i, d).GetJerkOptimalConnection(s0.position[d], s0.velocity[d], s0.acceleration[d], s1.position[d], s1.velocity[d],
                                                         s1.acceleration[d], para[i + 1] - para[i]);
            } else {
                (*spline)(i, d).set_zero();
            }
        }
...
template <int N_DEG, int N_DIM>
ErrorType SplineGenerator<N_DEG, N_DIM>::GetSplineFromFreeStateVec(const std::vector<decimal_t> &para, const vec_E<FreeState> &free_state_vec,
                                                                   SplineType *spline) {
    if (N_DEG < 5) {
        printf("[GetSplineFromStateVec]To fit state vec we need at least 5 "
               "degree.\n");
...
template <int N_DEG, int N_DIM>
ErrorType SplineGenerator<N_DEG, N_DIM>::GetBezierSplineUsingCorridor(const vec_E<SpatioTemporalSemanticCubeNd<N_DIM>> &cubes,
                                                                      const vec_E<Vecf<N_DIM>> &start_constraints,
                                                                      const vec_E<Vecf<N_DIM>> &end_constraints,
                                                                      const std::vector<decimal_t> &ref_stamps, const vec_E<Vecf<N_DIM>> &ref_points,
                                                                      const decimal_t &weight_proximity, BezierSplineType *bezier_spline) {
    ...
    Eigen::SparseMatrix<double, Eigen::RowMajor> P(total_num_vals, total_num_vals);
    P.reserve(Eigen::VectorXi::Constant(N_DIM * num_segments * num_order, num_order));
    ...
    if (!ref_stamps.empty()) {
        ...
        for (int i = 0; i < num_ref_samples; i++) {
            if (ref_stamps[i] < cubes[0].t_lb || ref_stamps[i] > cubes[num_segments - 1].t_ub)
                continue;
            ...
            for (int d = 0; d < N_DIM; d++) {
                for (int j = 0; j < num_order; j++) {
                    ...
                    for (int k = 0; k < num_order; k++) {
                        idy = d * num_segments * num_order + n * num_order + k;
                        P.coeffRef(idx, idy) +=
                            s * s * nchoosek(N_DEG, j) * nchoosek(N_DEG, k) * pow(t / s, j + k) * pow(1 - t / s, 2 * N_DEG - j - k);
                    }
                }
            }
        }
    ...
    int num_end_eq_constraints = static_cast<int>(end_constraints.size()) * N_DIM; // exclude s position constraint
    int total_num_eq_constraints = num_continuity_constraints + num_start_eq_constraints + num_end_eq_constraints;
    Eigen::SparseMatrix<double, Eigen::RowMajor> A(total_num_eq_constraints, N_DIM * num_segments * num_order);
    ...
    for (int i = 0; i < num_segments; i++) {
        ...
        total_num_ineq += (static_cast<int>(cubes[i].v_ub.size())) * (num_order - 1);
        total_num_ineq += (static_cast<int>(cubes[i].a_ub.size())) * (num_order - 2);
    }
...
ErrorType SplineGenerator<N_DEG, N_DIM>::GetBezierSplineUsingCorridor(const vec_E<SpatioTemporalSemanticCubeNd<N_DIM>> &cubes,
                                                                      const vec_E<Vecf<N_DIM>> &start_constraints,
                                                                      const vec_E<Vecf<N_DIM>> &end_constraints, BezierSplineType *bezier_spline) {
    int num_segments = static_cast<int>(cubes.size());
    int num_order = N_DEG + 1;
    int derivative_degree = 3;
