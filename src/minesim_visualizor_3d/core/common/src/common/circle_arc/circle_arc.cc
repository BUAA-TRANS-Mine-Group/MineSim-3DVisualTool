...
namespace common {
CircleArc::CircleArc(const Vec3f &start_state, const double &curvature, const double &arc_length)
    : start_state_(start_state), curvature_(curvature), arc_length_(arc_length) {
    if (curvature_ != 0.0) {
        is_arc_ = true;
...
double CircleArc::x_offs_d0(const double s, const double offs) const { return x_d0(s) + offs * nx_d0(s); }

double CircleArc::y_offs_d0(const double s, const double offs) const { return y_d0(s) + offs * ny_d0(s); }

double CircleArc::theta_offs_d0(const double s, const double offs) const { return theta_d0(s); }

void CircleArc::GetSampledStates(const double s_step, std::vector<Vec3f> *p_sampled_states) const {
    // Calculate sampled state for straight line
    for (double length = 0.0; fabs(length) < fabs(arc_length_); length += s_step) {
        Vec3f sampled_state;
