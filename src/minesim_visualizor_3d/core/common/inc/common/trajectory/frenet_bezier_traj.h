...
class FrenetBezierTrajectory : public FrenetTrajectory {
  public:
    ...
    FrenetBezierTrajectory(const BezierTrajectory &bezier_spline, const StateTransformer &stf)
    ...
    ErrorType GetFrenetState(const decimal_t &t, FrenetState *fs) const override {
        ...
        fs->time_stamp = t;
        fs->Load(Vec3f(pos[0], vel[0], acc[0]), Vec3f(pos[1], vel[1], acc[1]), common::FrenetState::kInitWithDt);
        if (!fs->is_ds_usable) {
