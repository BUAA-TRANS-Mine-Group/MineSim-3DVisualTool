...
ErrorType ConfigLoader::ParseAgentConfig(AgentConfigInfo *p_agent_config) {
    printf("\n#log#[ConfigLoader] Loading vehicle set\n");
    ...
    for (int i = 0; i < num; ++i) {
        ...
        p_agent_config->obstacle_map_meta_info =
            common::GridMapMetaInfo(agent["obstacle_map_meta_info"]["width"].get<double>(), agent["obstacle_map_meta_info"]["height"].get<double>(),
                                    agent["obstacle_map_meta_info"]["resolution"].get<double>());
        p_agent_config->surrounding_search_radius = agent["surrounding_search_radius"].get<double>();
        p_agent_config->enable_openloop_prediction = agent["enable_openloop_prediction"].get<bool>();
