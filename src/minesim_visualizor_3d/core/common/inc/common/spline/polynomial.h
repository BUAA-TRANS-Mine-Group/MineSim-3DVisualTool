...
template <int N_DEG> class Polynomial {
  public:
    ...
    inline decimal_t J(decimal_t s, int d) const {
        if (d == 3) {
            ...
            return coeff_(0) * coeff_(0) / 20.0 * pow(s, 5) + coeff_(0) * coeff_(1) / 4 * pow(s, 4) +
                   (coeff_(1) * coeff_(1) + coeff_(0) * coeff_(2)) / 3 * pow(s, 3) + coeff_(1) * coeff_(2) * s * s + coeff_(2) * coeff_(2) * s;
        } else if (d == 2) {
            ...
            return coeff_(0) * coeff_(0) / 252 * pow(s, 7) + coeff_(0) * coeff_(1) / 36 * pow(s, 6) +
                   ...
                   (coeff_(2) * coeff_(2) / 3 + coeff_(1) * coeff_(3) / 3) * pow(s, 3) + coeff_(2) * coeff_(3) * s * s + coeff_(3) * coeff_(3) * s;
        } else {
            assert(false);
        }
        return 0.0;
     ...
     * @param S, duration
     */
    void GetJerkOptimalConnection(const decimal_t p1, const decimal_t dp1, const decimal_t ddp1, const decimal_t p2, const decimal_t dp2,
                                  const decimal_t ddp2, const decimal_t S) {
        assert(N_DEG >= 5);
        Vecf<6> b;
     ...
     */
    void print() const { std::cout << std::fixed << std::setprecision(7) << coeff_.transpose() << std::endl; }
