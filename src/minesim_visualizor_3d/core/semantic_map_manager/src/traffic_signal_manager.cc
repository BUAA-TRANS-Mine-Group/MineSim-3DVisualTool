...
ErrorType TrafficSignalManager::GetSpeedLimit(const State &state, const Lane &lane, decimal_t *speed_limit) const {
    ...
    for (auto &speed_limit : speed_limit_list_) {
        ...
        if (CheckIntersectionTypeWithSignal(ref_fs, lane, speed_limit, &intersection_type, &dist_to_startpt, &dist_to_endpt) != kSuccess) {
            continue;
        }
        if (intersection_type != kNotIntersect) {
            decimal_t effect_speed_limit_dist =
                state.velocity > speed_limit.max_velocity()
                    ? fabs(speed_limit.max_velocity() * speed_limit.max_velocity() - state.velocity * state.velocity) / (2.0 * acc_esti)
                    : 0.0;
            ...
            if ((intersection_type == kSignalAhead && dist_to_startpt < effect_speed_limit_dist) || intersection_type == kSignalControlled) {
                limit = limit > speed_limit.max_velocity() ? speed_limit.max_velocity() : limit;
            }
        }
    }
    *speed_limit = limit;
...
ErrorType TrafficSignalManager::CheckIntersectionTypeWithSignal(const common::FrenetState &fs, const Lane &lane, const common::TrafficSignal &signal,
    ...
    if (stf.GetFrenetPointFromPoint(signal.start_point(), &start_pt_fs) != kSuccess) {
        return kWrongStatus;
    }
    ...
    if (start_pt_fs[0] < end_pt_fs[0] + kEPS) {
        if (start_pt_fs[1] + lateral_range(1) > 0.0 && start_pt_fs[1] + lateral_range(0) < 0.0 && end_pt_fs[1] + lateral_range(1) > 0.0 &&
...
ErrorType TrafficSignalManager::GetTrafficStoppingState(const State &state, const Lane &lane, State *stopping_state) const { return kSuccess; }
