...
OrientedBoundingBox2D::OrientedBoundingBox2D(const decimal_t x_, const decimal_t y_, const decimal_t angle_, const decimal_t width_,
...
bool ShapeUtils::CheckIfOrientedBoundingBoxIntersect(const OrientedBoundingBox2D &obb_a, const OrientedBoundingBox2D &obb_b) {
...
ErrorType ShapeUtils::GetVerticesOfOrientedBoundingBox(const OrientedBoundingBox2D &obb, vec_E<Vecf<2>> *vertices) {
    //
    //    corner2   corner1
    // y<--   ________   x
    //        |      |   ^
    ...
    Vecf<2> corner1(obb.x + 0.5 * obb.length * cos_theta + 0.5 * obb.width * sin_theta,
                    obb.y + 0.5 * obb.length * sin_theta - 0.5 * obb.width * cos_theta);
    Vecf<2> corner2(obb.x + 0.5 * obb.length * cos_theta - 0.5 * obb.width * sin_theta,
                    obb.y + 0.5 * obb.length * sin_theta + 0.5 * obb.width * cos_theta);
    Vecf<2> corner3(obb.x - 0.5 * obb.length * cos_theta - 0.5 * obb.width * sin_theta,
                    obb.y - 0.5 * obb.length * sin_theta + 0.5 * obb.width * cos_theta);
    Vecf<2> corner4(obb.x - 0.5 * obb.length * cos_theta + 0.5 * obb.width * sin_theta,
                    obb.y - 0.5 * obb.length * sin_theta - 0.5 * obb.width * cos_theta);
...
ErrorType ShapeUtils::GetPerpendicularAxesOfOrientedBoundingBox(const vec_E<Vecf<2>> &vertices, vec_E<Vecf<2>> *axes) {
...
ErrorType ShapeUtils::GetProjectionOnAxis(const vec_E<Vecf<2>> &vertices, const Vecf<2> &axis, Vecf<2> *proj) {
...
ErrorType ShapeUtils::GetPerpendicularAxisOfOrientedBoundingBox(const vec_E<Vecf<2>> &vertices, const int index, Vecf<2> *axis) {
...
ErrorType ShapeUtils::GetOverlapLength(const Vecf<2> a, const Vecf<2> b, decimal_t *len) {
    if (a.x() > b.y() || a.y() < b.x()) {
        *len = 0.0;
        return kSuccess;
    }
...
ErrorType ShapeUtils::GetCvPoint2iVecUsingCommonPoint2iVec(const std::vector<Point2i> &pts_in, std::vector<cv::Point2i> *pts_out) {
    int num = pts_in.size();
    pts_out->resize(num);
    for (int i = 0; i < num; ++i) {
...
ErrorType ShapeUtils::GetCvPoint2iUsingCommonPoint2i(const Point2i &pt_in, cv::Point2i *pt_out) {
    pt_out->x = pt_in.x;
    pt_out->y = pt_in.y;
    return kSuccess;
}
