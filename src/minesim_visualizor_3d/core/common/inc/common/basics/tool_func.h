 ...
 * @param tmp_result
 */
void GetResultInVector(const std::vector<std::vector<int>> &vec, const int &N, std::vector<int> *tmp, std::vector<std::vector<int>> *tmp_result);
/**
 ...
 * @param res
 */
void GetAllCombinations(const std::vector<std::vector<int>> &vec_in, std::vector<std::vector<int>> *res);
 ...
 */
template <typename T> std::string GetStringByValueWithPrecision(const T &val, const int &pre) {
...
template <typename T> void GetRangeVector(const T &lb, const T &ub, const T &step, const bool &if_inc_tail, std::vector<T> *vec) {
    vec->clear();
