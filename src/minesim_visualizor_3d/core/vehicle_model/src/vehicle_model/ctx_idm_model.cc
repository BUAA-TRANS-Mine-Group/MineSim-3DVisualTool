...
ContextIntelligentDriverModel::ContextIntelligentDriverModel() { UpdateInternalState(); }

ContextIntelligentDriverModel::ContextIntelligentDriverModel(const IdmParam &idm_parm, const CtxParam &ctx_param)
...
void ContextIntelligentDriverModel::operator()(const InternalState &x, InternalState &dxdt, const double dt) {
    ...
    decimal_t acc_idm;
    common::IntelligentDriverModel::GetAccDesiredAcceleration(idm_param_, idm_state, &acc_idm);
    acc_idm = std::max(acc_idm, -std::min(idm_param_.kHardBrakingDeceleration, cur_state.v / dt));

    decimal_t v_ref = cur_state.v_target + ctx_param_.k_s * (cur_state.s_target - cur_state.s);
...
const ContextIntelligentDriverModel::CtxIdmState &ContextIntelligentDriverModel::state(void) const { return state_; }

void ContextIntelligentDriverModel::set_state(const ContextIntelligentDriverModel::CtxIdmState &state) {
    state_ = state;
    UpdateInternalState();
}
