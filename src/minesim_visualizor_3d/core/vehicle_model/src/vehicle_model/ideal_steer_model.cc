...
IdealSteerModel::IdealSteerModel(double wheelbase_len, double max_lon_acc, double max_lon_dec, double max_lon_acc_jerk, double max_lon_dec_jerk,
                                 double max_lat_acc, double max_lat_jerk, double max_steering_angle, double max_steer_rate, double max_curvature)
    : wheelbase_len_(wheelbase_len), max_lon_acc_(max_lon_acc), max_lon_dec_(max_lon_dec), max_lon_acc_jerk_(max_lon_acc_jerk),
      max_lon_dec_jerk_(max_lon_dec_jerk), max_lat_acc_(max_lat_acc), max_lat_jerk_(max_lat_jerk), max_steering_angle_(max_steering_angle),
...
void IdealSteerModel::TruncateControl(const decimal_t &dt) {
    ...
    desired_lon_jerk = truncate(desired_lon_jerk, -max_lon_dec_jerk_, max_lon_acc_jerk_);
    ...
    desired_lat_acc_ = pow(control_.velocity, 2) * (tan(control_.steer) / wheelbase_len_);
    ...
    control_.steer = atan(desired_lat_acc_ * wheelbase_len_ / std::max(pow(control_.velocity, 2), 0.1 * kBigEPS));
    desired_steer_rate_ = normalize_angle(control_.steer - state_.steer) / dt;
    desired_steer_rate_ = truncate(desired_steer_rate_, -max_steer_rate_, max_steer_rate_);
...
void IdealSteerModel::Step(double dt) {
    ...
    control_.steer = truncate(control_.steer, -max_steering_angle_, max_steering_angle_);
    TruncateControl(dt);
    desired_lon_acc_ = (control_.velocity - state_.velocity) / dt;
    desired_steer_rate_ = normalize_angle(control_.steer - state_.steer) / dt;
...
void IdealSteerModel::set_control(const Control &control) { control_ = control; }

const IdealSteerModel::State &IdealSteerModel::state(void) const { return state_; }
