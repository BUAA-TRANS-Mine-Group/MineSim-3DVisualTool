...
ErrorType MobilBehaviorPrediction::RemapGainsToProb(const bool is_lcl_safe, const decimal_t mobil_gain_left, const bool is_lcr_safe,
                                                    const decimal_t mobil_gain_right, ProbDistOfLatBehaviors *res) {
    ...
    if (is_lcl_safe && is_lcr_safe) {
        ...
        decimal_t gain = normalize_with_bound(mobil_gain_left, lower_bound, upper_bound, 0.0, 1.0);
        ...
        decimal_t gain = normalize_with_bound(mobil_gain_right, lower_bound, upper_bound, 0.0, 1.0);
...
ErrorType MobilBehaviorPrediction::LateralBehaviorPrediction(const Vehicle &vehicle, const vec_E<Lane> &lanes,
    ...
    if (lk_lane.IsValid()) {
        ...
        if (MobilLaneChangingModel::GetMobilAccChangesOnCurrentLane(ego_frenet_state, leading_vehicle, leading_fs, following_vehicle, following_fs,
                                                                    &acc_o, &acc_o_tilda, &acc_c) != kSuccess) {
    ...
    if (lcl_lane.IsValid()) {
        ...
        MobilLaneChangingModel::GetMobilAccChangesOnTargetLane(projected_frenet_state, leading_vehicle, leading_fs, following_vehicle, following_fs,
                                                               &is_lcl_safe, &acc_n, &acc_n_tilda, &acc_c_tilda);
        if (is_lcl_safe) {
    ...
    if (lcr_lane.IsValid()) {
        ...
        MobilLaneChangingModel::GetMobilAccChangesOnTargetLane(projected_frenet_state, leading_vehicle, leading_fs, following_vehicle, following_fs,
                                                               &is_lcr_safe, &acc_n, &acc_n_tilda, &acc_c_tilda);
        if (is_lcr_safe) {
            decimal_t d_a_c = acc_c_tilda - acc_c;
            decimal_t d_a_n = acc_n_tilda - acc_n;
            decimal_t d_a_o = acc_o_tilda - acc_o;
    ...
    printf("[Mobil]%d, Left gain: %.3lf, Right gain: %.3lf\n", vehicle.id(), mobil_gain_left, mobil_gain_right);
    return kSuccess;
}
