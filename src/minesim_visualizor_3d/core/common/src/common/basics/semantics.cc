...
Vehicle::Vehicle(const VehicleParam &param, const State &state) : param_(param), state_(state) {}

Vehicle::Vehicle(const int &id, const VehicleParam &param, const State &state) : id_(id), param_(param), state_(state) {}

Vehicle::Vehicle(const int &id, const std::string &subclass, const VehicleParam &param, const State &state)
    : id_(id), subclass_(subclass), param_(param), state_(state) {}

Vec3f Vehicle::Ret3DofState() const { return Vec3f(state_.vec_position(0), state_.vec_position(1), state_.angle); }
...
VehicleControlSignal::VehicleControlSignal(double acc, double steer_rate) : acc(acc), steer_rate(steer_rate), is_openloop(false) {}

VehicleControlSignal::VehicleControlSignal(common::State state) : acc(0.0), steer_rate(0.0), is_openloop(true), state(state) {}

GridMapMetaInfo::GridMapMetaInfo() {}

GridMapMetaInfo::GridMapMetaInfo(const int w, const int h, const double res) : width(w), height(h), resolution(res) {
...
GridMapND<T, N_DIM>::GridMapND(const std::array<int, N_DIM> &dims_size, const std::array<decimal_t, N_DIM> &dims_resolution,
...
template <typename T, int N_DIM> ErrorType GridMapND<T, N_DIM>::GetValueUsingCoordinate(const std::array<int, N_DIM> &coord, T *val) const {
...
template <typename T, int N_DIM> ErrorType GridMapND<T, N_DIM>::GetValueUsingGlobalPosition(const std::array<decimal_t, N_DIM> &p_w, T *val) const {
...
template <typename T, int N_DIM>
ErrorType GridMapND<T, N_DIM>::CheckIfEqualUsingGlobalPosition(const std::array<decimal_t, N_DIM> &p_w, const T &val_in, bool *res) const {
    std::array<int, N_DIM> coord = GetCoordUsingGlobalPosition(p_w);
    T val;
...
template <typename T, int N_DIM>
ErrorType GridMapND<T, N_DIM>::CheckIfEqualUsingCoordinate(const std::array<int, N_DIM> &coord, const T &val_in, bool *res) const {
    T val;
    if (GetValueUsingCoordinate(coord, &val) != kSuccess) {
        *res = false;
    } else {
...
template <typename T, int N_DIM> ErrorType GridMapND<T, N_DIM>::SetValueUsingCoordinate(const std::array<int, N_DIM> &coord, const T &val) {
    if (!CheckCoordInRange(coord)) {
        // printf("[GridMapND] Out of range\n");
        return kWrongStatus;
    }
...
template <typename T, int N_DIM> ErrorType GridMapND<T, N_DIM>::SetValueUsingGlobalPosition(const std::array<decimal_t, N_DIM> &p_w, const T &val) {
    std::array<int, N_DIM> coord = GetCoordUsingGlobalPosition(p_w);
...
template <typename T, int N_DIM>
std::array<decimal_t, N_DIM> GridMapND<T, N_DIM>::GetRoundedPosUsingGlobalPosition(const std::array<decimal_t, N_DIM> &p_w) const {
    std::array<int, N_DIM> coord = {};
    for (int i = 0; i < N_DIM; ++i) {
        coord[i] = std::round((p_w[i] - origin_[i]) / dims_resolution_[i]);
    }
...
template <typename T, int N_DIM>
ErrorType GridMapND<T, N_DIM>::GetGlobalPositionUsingCoordinate(const std::array<int, N_DIM> &coord, std::array<decimal_t, N_DIM> *p_w) const {
    auto ptr = p_w->data();
    for (int i = 0; i < N_DIM; ++i) {
...
template <typename T, int N_DIM>
ErrorType GridMapND<T, N_DIM>::GetCoordUsingGlobalMetricOnSingleDim(const decimal_t &metric, const int &i, int *idx) const {
    *idx = std::round((metric - origin_[i]) / dims_resolution_[i]);
    return kSuccess;
}

template <typename T, int N_DIM>
ErrorType GridMapND<T, N_DIM>::GetGlobalMetricUsingCoordOnSingleDim(const int &idx, const int &i, decimal_t *metric) const {
    *metric = idx * dims_resolution_[i] + origin_[i];
    return kSuccess;
}

template <typename T, int N_DIM> bool GridMapND<T, N_DIM>::CheckCoordInRange(const std::array<int, N_DIM> &coord) const {
    for (int i = 0; i < N_DIM; ++i) {
...
template <typename T, int N_DIM> bool GridMapND<T, N_DIM>::CheckCoordInRangeOnSingleDim(const int &idx, const int &i) const {
    return (idx >= 0) && (idx < dims_size_[i]);
}

template <typename T, int N_DIM> int GridMapND<T, N_DIM>::GetMonoIdxUsingNDimIdx(const std::array<int, N_DIM> &idx) const {
    int mono_idx = 0;
    for (int i = 0; i < N_DIM; ++i) {
...
template <typename T, int N_DIM> std::array<int, N_DIM> GridMapND<T, N_DIM>::GetNDimIdxUsingMonoIdx(const int &idx) const {
...
template <typename T, int N_DIM> ErrorType GridMapND<T, N_DIM>::SetNDimSteps(const std::array<int, N_DIM> &dims_size) {
    int step = 1;
    for (int i = 0; i < N_DIM; ++i) {
        dims_step_[i] = step;
...
template <typename T, int N_DIM> ErrorType GridMapND<T, N_DIM>::SetDataSize(const std::array<int, N_DIM> &dims_size) {
    int total_ele_num = 1;
    for (int i = 0; i < N_DIM; ++i) {
...
TrafficSignal::TrafficSignal()
    : start_point_(Vec2f::Zero()), end_point_(Vec2f::Zero()), valid_time_(Vec2f(0.0, std::numeric_limits<decimal_t>::max())),
      vel_range_(Vec2f::Zero()), lateral_range_(Vec2f(-1.75, 1.75)) {}

TrafficSignal::TrafficSignal(const Vec2f &start_point, const Vec2f &end_point, const Vec2f &valid_time, const Vec2f &vel_range)
    : start_point_(start_point), end_point_(end_point), valid_time_(valid_time), vel_range_(vel_range), lateral_range_(Vec2f(-1.75, 1.75)) {}

void TrafficSignal::set_start_point(const Vec2f &start_point) { start_point_ = start_point; }

void TrafficSignal::set_end_point(const Vec2f &end_point) { end_point_ = end_point; }

void TrafficSignal::set_valid_time_til(const decimal_t max_valid_time) { valid_time_(1) = max_valid_time; }

void TrafficSignal::set_valid_time_begin(const decimal_t min_valid_time) { valid_time_(0) = min_valid_time; }

void TrafficSignal::set_valid_time(const Vec2f &valid_time) { valid_time_ = valid_time; }

void TrafficSignal::set_vel_range(const Vec2f &vel_range) { vel_range_ = vel_range; }

void TrafficSignal::set_lateral_range(const Vec2f &lateral_range) { lateral_range_ = lateral_range; }

void TrafficSignal::set_max_velocity(const decimal_t max_velocity) { vel_range_(1) = max_velocity; }
...
SpeedLimit::SpeedLimit(const Vec2f &start_point, const Vec2f &end_point, const Vec2f &vel_range)
    : TrafficSignal(start_point, end_point, Vec2f(0.0, std::numeric_limits<decimal_t>::max()), vel_range) {}

StoppingSign::StoppingSign(const Vec2f &start_point, const Vec2f &end_point)
    : TrafficSignal(start_point, end_point, Vec2f(0.0, std::numeric_limits<decimal_t>::max()), Vec2f::Zero()) {}

void TrafficLight::set_type(const Type &type) { type_ = type; }

TrafficLight::Type TrafficLight::type() const { return type_; }

ErrorType SemanticsUtils::GetOrientedBoundingBoxForVehicleUsingState(const VehicleParam &param, const State &s, OrientedBoundingBox2D *obb) {
...
ErrorType SemanticsUtils::GetVehicleVertices(const VehicleParam &param, const State &state, vec_E<Vec2f> *vertices) {
...
ErrorType SemanticsUtils::InflateVehicleBySize(const Vehicle &vehicle_in, const decimal_t delta_w, const decimal_t delta_l, Vehicle *vehicle_out) {
