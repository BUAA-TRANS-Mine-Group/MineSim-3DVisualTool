...
class PhySimulation {
  public:
    PhySimulation();
    PhySimulation(const std::string &vehicle_set_path, const std::string &map_path, const std::string &lane_net_path, bool use_actual_heights);
    ~PhySimulation() {}
    ...
    bool UpdateSimulatorUsingSignalSet(const common::VehicleControlSignalSet &signal_set, const decimal_t &dt);

  private:
    bool GetDataFromArenaLoader(bool use_actual_heights);

    bool SetupVehicleModelForVehicleSet();

    bool UpdateVehicleStates(const common::VehicleControlSignalSet &signal_set, const decimal_t &dt);

    ArenaLoader *p_arena_loader_;
    bool use_actual_heights_;
