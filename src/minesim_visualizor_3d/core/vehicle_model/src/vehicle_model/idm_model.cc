...
IntelligentDriverModel::IntelligentDriverModel(const Param &parm) : param_(parm) { UpdateInternalState(); }
...
void IntelligentDriverModel::Linear(const InternalState &x, const double dt, InternalState *x_out) {
    State cur_state;
    cur_state.s = x[0];
    cur_state.v = x[1];
    ...
    decimal_t acc;
    common::IntelligentDriverModel::GetIIdmDesiredAcceleration(param_, cur_state, &acc);

    std::cout << "acc = " << acc << std::endl;
    acc = std::max(acc, -std::min(param_.kHardBrakingDeceleration, cur_state.v / dt));
...
void IntelligentDriverModel::operator()(const InternalState &x, InternalState &dxdt, const double dt) {
    State cur_state;
    cur_state.s = x[0];
    cur_state.v = x[1];
    cur_state.s_front = x[2];
    ...
    decimal_t acc;
    common::IntelligentDriverModel::GetAccDesiredAcceleration(param_, cur_state, &acc);
    acc = std::max(acc, -std::min(param_.kHardBrakingDeceleration, cur_state.v / dt));
    dxdt[0] = cur_state.v;
    dxdt[1] = acc;
...
const IntelligentDriverModel::State &IntelligentDriverModel::state(void) const { return state_; }

void IntelligentDriverModel::set_state(const IntelligentDriverModel::State &state) {
    state_ = state;
    UpdateInternalState();
}
